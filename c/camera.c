...
Camera get_camera(Vec3 lookfrom, Vec3 lookat, Vec3 vup, float vfov, float aspect_ratio,
  ...
  Vec3 lower_left_corner = subtract(subtract(subtract(origin, half_width_vec), half_height_vec), multiplyf(w, focus_dist));


  Vec3 horizontal = multiplyf(u, 2 * half_width * focus_dist);
  Vec3 vertical = multiplyf(v, 2 * half_height * focus_dist);

  // The camera is defined as an origin point, a vector anchoring a corner of the rendering plane,
  // and vectors from that corner to the horizontal and vertical extent of that canvas
  // This allows ray construction across the plane
